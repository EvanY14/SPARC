function sm_mpg_integral_tracking(integrator)
    ...
    if !(status in ("solved", "solved inaccurate"))
        @warn "Integral SM-MPG OSQP solve returned status $status; falling back to reference control"
        du_star = zeros(nz)
    end
...
const sm_mpg_integral = sm_mpg_integral_tracking
