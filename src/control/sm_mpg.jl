...
function sm_mpg_tuning_snapshot()
    return (
        lambda = _SM_MPG_LAMBDA[],
        sliding_gain = _SM_MPG_SLIDING_GAIN[],
        increment_gain = _SM_MPG_INCREMENT_GAIN[],
    )
end

function set_sm_mpg_tuning!(; lambda=nothing, sliding_gain=nothing, increment_gain=nothing)
    if lambda !== nothing
        λ = Float64(lambda)
        (isfinite(λ) && 0.0 <= λ && λ <= 1.5) || throw(ArgumentError("lambda must be finite and between 0.0 and 1.5"))
        _SM_MPG_LAMBDA[] = λ
    end
    if sliding_gain !== nothing
        gain = Float64(sliding_gain)
        (isfinite(gain) && gain >= 0.0) || throw(ArgumentError("sliding_gain must be finite and nonnegative"))
        _SM_MPG_SLIDING_GAIN[] = gain
    end
    if increment_gain !== nothing
        ...
        _SM_MPG_INCREMENT_GAIN[] = gain
    end
    return sm_mpg_tuning_snapshot()
end
...
function _sm_mpg_add_tracking_cost!(
    ...
    W_slide = _mpg_stage_state_normalized_weight_matrix()

    for k in 1:N
        # Homework 3, Question 4 sliding variable: s_k = e_{k+1} - λ e_k.
        SΦ = C_slide * (Φ[k + 1] .- λ .* Φ[k])
        SΨ = C_slide * (Ψ[k + 1] .- λ .* Ψ[k])
        weight = _SM_MPG_SLIDING_GAIN[] * Float64(interval_weights[k])
...
function sm_mpg_tracking(integrator)
    ...
    _sm_mpg_add_tracking_cost!(P_qp, q_qp, Φ, Ψ, dx0, step_sizes)
    _add_mpg_actual_increment_cost!(
        ...
        prev_u,
        step_sizes;
        RΔ = _mpg_control_cost_matrix(),
        gain = _SM_MPG_INCREMENT_GAIN[],
    )
